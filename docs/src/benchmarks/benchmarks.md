...
The package ships a benchmark suite (under `benchmark/`) for the shallow-net variational
integrator `ShallowNet`. It runs each of several test problems through a large grid of
integrator configurations and records, for every case,
...
command-line argument or from the `SHALLOWNET_BENCH_PRESET` environment variable.
...
which writes `benchmark/results/shallownet_benchmark.md`. The reporting step reads the
...
![Convergence success rate by problem](figures/shallownet_benchmark_convergence_problem.png)

![Convergence success rate by solver strategy](figures/shallownet_benchmark_convergence_solver.png)

![Convergence success rate by solver and precision](figures/shallownet_benchmark_convergence_heatmap.png)
...
![Accuracy versus timestep](figures/shallownet_benchmark_accuracy_vs_dt.png)

![Energy drift versus timestep](figures/shallownet_benchmark_energy_drift_vs_dt.png)

![Run time versus timestep](figures/shallownet_benchmark_runtime_vs_dt.png)

![Nonlinear iterations versus timestep](figures/shallownet_benchmark_iterations_vs_dt.png)
...
`benchmark/shallownet_benchmark_common.jl` and `benchmark/shallownet_report.jl`.
