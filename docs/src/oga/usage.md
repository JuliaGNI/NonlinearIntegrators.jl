...
`ShallowNet` only, and it carries none of the guard rails.

```julia
using NonlinearIntegrators, QuadratureRules

relu3(x) = max(zero(x), x)^3                       # float-generic — see Precision
basis = ShallowNetBasis{Float64}(relu3, 4)     # S = 4 hidden neurons
...
|---|---|---|---|
| `ShallowNet` | — | [`NoSymmetry`](@ref) | [`OGA1d`](@ref) |
| `ShallowNetAutodiff` | `t(1-t)`, target minus the linear part | [`NoSymmetry`](@ref) | [`OGA1dNormalized`](@ref) |
| `ShallowNetReversible` | — | [`MirrorPairs`](@ref) | [`OGA1d`](@ref) |
| `ShallowNetAutodiffReversible` | `t(1-t)`, target minus the linear part | [`SharedMirrorPairs`](@ref) | [`OGA1d`](@ref) |

`ShallowNetAutodiff` is the odd one out, and deliberately so: its greedy step ranks
...
linear part comes from — `ShallowNetAutodiff` uses the last label (i.e. the initial-trajectory
integrator's estimate), `ShallowNetAutodiffReversible` uses the cache's endpoint estimate `q̃` —
