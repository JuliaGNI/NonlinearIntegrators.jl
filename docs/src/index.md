...
- **`OneLayerMethod`** — single-hidden-layer ansatz. Includes `NonLinear_OneLayer_GML`,
  `Hardcode_int`, `Time_reversible_OneLayer`, and `Time_Reversible_Hardcode`. All use
  the Orthogonal Greedy Algorithm (`OGA1d`) to initialise the network weights at each
  step.
