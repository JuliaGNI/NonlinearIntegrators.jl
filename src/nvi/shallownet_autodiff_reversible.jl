...
function GeometricIntegratorsBase.issymmetric(::Union{
        ShallowNetAutodiffReversible, Type{<:ShallowNetAutodiffReversible}})
    true
end
...
function GeometricIntegratorsBase.Cache{ST}(problem::AbstractProblemIODE,
        method::ShallowNetAutodiffReversible; kwargs...) where {ST}
...
@inline GeometricIntegratorsBase.CacheType(ST, problem::AbstractProblemIODE,
    method::ShallowNetAutodiffReversible) = AutodiffShallowNetCache{ST}
...
function initial_trajectory!(
        sol, history, params, int::GeometricIntegrator{<:ShallowNetAutodiffReversible},
...
function GeometricIntegratorsBase.components!(x::AbstractVector{ST}, sol, params,
        int::GeometricIntegrator{<:ShallowNetAutodiffReversible}) where {ST}
    local D = length(cache(int).q̃)
    local S = nbasis(method(int))
...
function update_solution!(
        sol, params, int::GeometricIntegrator{<:ShallowNetAutodiffReversible},
        ::Type{DT}) where {DT}
    local D = length(cache(int).q̃)
    local quad_nodes = QuadratureRules.nodes(int.method.quadrature)
    local P = cache(int).P
