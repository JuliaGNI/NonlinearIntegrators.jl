struct NonLinear_DenseNet_Lux{T, NBASIS,NHIDDEN, NNODES, basisType <: Basis{T}} <: DenseNetMethod
    basis::basisType
    ...
    function NonLinear_DenseNet_Lux(basis::Basis{T}, quadrature::QuadratureRule{T};nstages::Int = 10,show_status::Bool=true,training_epochs::Int=50000) where {T}
...
CompactBasisFunctions.basis(method::NonLinear_DenseNet_Lux) = method.basis
quadrature(method::NonLinear_DenseNet_Lux) = method.quadrature
CompactBasisFunctions.nbasis(method::NonLinear_DenseNet_Lux) = method.basis.S
nnodes(method::NonLinear_DenseNet_Lux) = QuadratureRules.nnodes(method.quadrature)
activation(method::NonLinear_DenseNet_Lux) = method.basis.activation
nstages(method::NonLinear_DenseNet_Lux) = method.nstages

isexplicit(::Union{NonLinear_DenseNet_Lux, Type{<:NonLinear_DenseNet_Lux}}) = false
isimplicit(::Union{NonLinear_DenseNet_Lux, Type{<:NonLinear_DenseNet_Lux}}) = true
issymmetric(::Union{NonLinear_DenseNet_Lux, Type{<:NonLinear_DenseNet_Lux}}) = missing
issymplectic(::Union{NonLinear_DenseNet_Lux, Type{<:NonLinear_DenseNet_Lux}}) = true


default_solver(::NonLinear_DenseNet_Lux) = Newton()
# default_iguess(::NonLinear_DenseNet_Lux) = HermiteExtrapolation()# HarmonicOscillator
default_iguess(::NonLinear_DenseNet_Lux) = MidpointExtrapolation()#CoupledHarmonicOscillator
default_iguess_integrator(::NonLinear_DenseNet_Lux) = ImplicitMidpoint()

struct NonLinear_DenseNet_LuxCache{ST,D,S₁,S,R,N} <: IODEIntegratorCache{ST,D}
    x::Vector{ST}
    ...
    ps::Vector{@NamedTuple{
        layer_1::@NamedTuple{weight::Matrix{ST}, bias::Matrix{ST}}, 
        layer_2::@NamedTuple{weight::Matrix{ST}, bias::Matrix{ST}},
        layer_3::@NamedTuple{weight::Matrix{ST},},
        }}
    st::@NamedTuple{layer_1::@NamedTuple{}, layer_2::@NamedTuple{},layer_3::@NamedTuple{},}
    ...
    function NonLinear_DenseNet_LuxCache{ST,D,S₁,S,R,N}() where {ST,D,S₁,S,R,N}
        ...
        ps = [(layer_1 = (weight = zeros(ST,S₁,1), bias = zeros(ST,S₁,1)), 
                layer_2 = (weight = zeros(ST,S₁,S), bias = zeros(ST,S,1)),
                layer_3 = (weight = zeros(ST,1,S),)
                )  
        for k in 1:D]
...
function GeometricIntegrators.Integrators.reset!(cache::NonLinear_DenseNet_LuxCache, t, q, p)
    copyto!(cache.q̄, q)
    copyto!(cache.p̄, p)
end

GeometricIntegrators.Integrators.nlsolution(cache::NonLinear_DenseNet_LuxCache) = cache.x

function GeometricIntegrators.Integrators.Cache{ST}(problem::AbstractProblemIODE, method::NonLinear_DenseNet_Lux; kwargs...) where {ST}
    NonLinear_DenseNet_LuxCache{ST, ndims(problem), method.basis.S₁,nbasis(method), nnodes(method),nstages(method)}(; kwargs...)
end

@inline GeometricIntegrators.Integrators.CacheType(ST, problem::AbstractProblemIODE, method::NonLinear_DenseNet_Lux) = NonLinear_DenseNet_LuxCache{ST, ndims(problem), method.basis.S₁,nbasis(method), nnodes(method),nstages(method)}

@inline function Base.getindex(c::NonLinear_DenseNet_LuxCache, ST::DataType)
...
function initial_guess_Extrapolation!(int::GeometricIntegrator{<:NonLinear_DenseNet_Lux})
    local network_inputs = method(int).network_inputs
    local network_labels = cache(int).network_labels
    local D = ndims(int)
    local h = int.problem.tstep
...
function initial_guess_networktraining!(int::GeometricIntegrator{<:NonLinear_DenseNet_Lux})
    local D = ndims(int)
    local S = nbasis(method(int))
    ...
    for k in 1:D
        ...
        tem_ps,tem_st=Lux.setup(Random.default_rng(),NN) #Random.seed!(1), create a temperoary ps,st 
        opt = Optimisers.Adam()
        st_opt = Optimisers.setup(opt, tem_ps)
        err = 0
        label = network_labels[:,k]'

        for ep in 1:nepochs
            gs = Zygote.gradient(p -> mse_loss(network_inputs,label,NN,p,tem_st)[1],tem_ps)[1]
            st_opt, tem_ps = Optimisers.update(st_opt, tem_ps, gs)
            err = mse_loss(network_inputs,label,NN,tem_ps,tem_st)[1]
        end
        ...
        st = tem_st
        for i in 1:S
            x[D*(i-1)+k] = ps[k][end].weight[i]
            for kk in 1:S₁
                x[D*(S+1)+D*(i-1)*S₁+D*(kk-1)+k] = ps[k][end-1].weight[i,kk]
            end
            x[D*(S+1+S₁*S)+D*(i-1)+k] = ps[k][end-1].bias[i]
        end
...
function GeometricIntegrators.Integrators.components!(x::AbstractVector{ST}, int::GeometricIntegrator{<:NonLinear_DenseNet_Lux}) where {ST}
    ...
    if ST != Float64 
        for k in 1:D
            for layers in 1:length(float_ps[k])-2
                ps[k][layers].weight[:] = float_ps[k][layers].weight[:]
                ps[k][layers].bias[:] = float_ps[k][layers].bias[:]
            end
        end
    end
    ...
    for d in 1:D
        for i in 1:S
            ps[d][end].weight[i]= x[D*(i-1)+d]
            for kk in 1:S₁
                ps[d][end-1].weight[i,kk] = x[D*(S+1)+D*(i-1)*S₁+D*(kk-1)+d]
            end
            ps[d][end-1].bias[i]= x[D*(S+1+S₁*S)+D*(i-1)+d]
        end
    end 
    ...
    for d in 1:D
        r₀[:,d] = NN[1:end-1]([0.0],(layer_1=ps[d][1],layer_2=ps[d][2]),(layer_1=st[1],layer_2=st[2]))[1]
        r₁[:,d] = NN[1:end-1]([1.0],(layer_1=ps[d][1],layer_2=ps[d][2]),(layer_1=st[1],layer_2=st[2]))[1]
        # for i in 1:S
        gs0 = Zygote.gradient(p->NN([0.0],p,st)[1][1],ps[d])[1]
        dqdWr₀[:,:,d] = gs0[end-1].weight[:]
        dqdbr₀[:,d] = gs0[end-1].bias[:]

        gs1 = Zygote.gradient(p->NN([1.0],p,st)[1][1],ps[d])[1]
        dqdWr₁[:,:,d] = gs1[end-1].weight[:]
        dqdbr₁[:,d] = gs1[end-1].bias[:]

        for j in eachindex(quad_nodes)
            m[j,:,d] = NN[1:end-1]([quad_nodes[j]],(layer_1=ps[d][1],layer_2=ps[d][2]),(layer_1=st[1],layer_2=st[2]))[1]
            a[j,:,d] = basis_first_order_central_difference(NN[1:end-1],(layer_1=ps[d][1],layer_2=ps[d][2]),(layer_1=st[1],layer_2=st[2]),quad_nodes[j])
            
            gsc = Zygote.gradient(p->NN([quad_nodes[j]],p,st)[1][1],ps[d])[1]
            dqdWc[j,:,:,d] = gsc[end-1].weight[:]
            dqdbc[j,:,d] = gsc[end-1].bias[:]

            vgs_f = Zygote.gradient(p->NN([quad_nodes[j]+ϵ],p,st)[1][1],ps[d])[1]
            vgs_b = Zygote.gradient(p->NN([quad_nodes[j]-ϵ],p,st)[1][1],ps[d])[1]

            dvdWc[j,:,:,d] = (vgs_f[end-1].weight[:] .- vgs_b[end-1].weight[:])/(2*ϵ)
            dvdbc[j,:,d] = (vgs_f[end-1].bias[:] .- vgs_b[end-1].bias[:])/(2*ϵ)
        end
    end
...
function GeometricIntegrators.Integrators.residual!(b::Vector{ST}, int::GeometricIntegrator{<:NonLinear_DenseNet_Lux}) where {ST}
    local D = ndims(int)
    local S = nbasis(method(int))
    local S₁ = int.method.basis.S₁
...
function GeometricIntegrators.Integrators.residual!(b::AbstractVector{ST}, x::AbstractVector{ST}, int::GeometricIntegrator{<:NonLinear_DenseNet_Lux}) where {ST}
...
function GeometricIntegrators.Integrators.update!(x::AbstractVector{DT}, int::GeometricIntegrator{<:NonLinear_DenseNet_Lux}) where {DT}
    # copy previous solution from solstep to cache
...
function GeometricIntegrators.Integrators.integrate_step!(int::GeometricIntegrator{<:NonLinear_DenseNet_Lux, <:AbstractProblemIODE})
    # copy previous solution from solstep to cache
...
function stages_compute!(int::GeometricIntegrator{<:NonLinear_DenseNet_Lux})
    local x = nlsolution(int)
    ...
    for d in 1:D
        ...
        stage_values[:,d] = NN(network_inputs,ps[d],st)[1][2:end]
    end
