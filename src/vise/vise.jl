...
struct VISE{T, NNODES, basisType <: Basis{T}, quadType} <: LODEMethod
    ...
    function VISE(basis::Basis{T}, quadrature, init_w::Vector{Vector{T}};
        ...
        new{T, NNODES, typeof(basis), typeof(quadrature)}(
            basis, quadrature, quad_weights, quad_nodes,
...
@inline GeometricIntegratorsBase.CacheType(ST, problem::AbstractProblemIODE, method::VISE) = VISECache{
    ST, nnodes(method)}
...
function GeometricIntegratorsBase.initial_guess!(sol, history, params, int::GeometricIntegrator{<:VISE})
    ...
    tem_ode = similar(problem, [zero(h), h], h / 100,
...
function GeometricIntegratorsBase.components!(
        x::AbstractVector{ST}, sol, params, int::GeometricIntegrator{<:VISE}) where {ST}
    local D = length(cache(int).q̃)
    local S = sum(method(int).basis.W_sizes)
    ...
    for d in 1:D
        for j in eachindex(quad_nodes)
            for p in eachindex(tem_W[d])
                dqdWc[d][j, p] = DQDW[d][p](tem_W[d], sol.t - timestep(int) +
                                                      quad_nodes[j] * timestep(int))
                dvdWc[d][j, p] = DVDW[d][p](tem_W[d], sol.t - timestep(int) +
                                                      quad_nodes[j] * timestep(int))
            end
        end
    ...
    for i in eachindex(Q)
        for d in eachindex(Q[i])
            Q[i][d] = q_expr[d](tem_W[d], sol.t - timestep(int) +
                                          quad_nodes[i] * timestep(int))
        end
    end
    ...
    for i in eachindex(V)
        for d in eachindex(V[i])
            V[i][d] = v_expr[d](tem_W[d], sol.t - timestep(int) +
                                          quad_nodes[i] * timestep(int))
    ...
    for i in eachindex(Q, V, P, F)
        equations(int).ϑ(
            P[i], sol.t - timestep(int) + quad_nodes[i] * timestep(int), Q[i], V[i], params)
        equations(int).f(
            F[i], sol.t - timestep(int) + quad_nodes[i] * timestep(int), Q[i], V[i], params)
    end
end

function GeometricIntegratorsBase.residual!(
        b::Vector{ST}, sol, params, int::GeometricIntegrator{<:VISE}) where {ST}
    local D = length(cache(int).q̃)
    local S = sum(method(int).basis.W_sizes)
    local W_sizes = method(int).basis.W_sizes
...
function GeometricIntegratorsBase.residual!(
        b::AbstractVector{ST}, x::AbstractVector{ST}, sol,
        params, int::GeometricIntegrator{<:VISE}) where {ST}
...
function GeometricIntegratorsBase.update!(
        sol, params, x::AbstractVector{DT}, int::GeometricIntegrator{<:VISE}) where {DT}
...
function GeometricIntegratorsBase.integrate_step!(
        sol, history, params, int::GeometricIntegrator{<:VISE, <:AbstractProblemIODE})
...
function record_finer_solution!(sol, int::GeometricIntegrator{<:VISE})
    ...
    for d in 1:D
        for i in eachindex(network_inputs)
            stage_values[i, d] = q_expr[d](tem_W[d], sol.t - timestep(int) +
                                                     network_inputs[i] * timestep(int))
        end
    end
end
...
function create_quadrature_points_derivative_vector(::Type{ST}, R::Int, D::Int, W_sizes::Vector{Int}) where {ST}
    [zeros(ST, R, W_sizes[d]) for d in 1:D]
end

function create_boundary_derivative_vector(::Type{ST}, D::Int, W_sizes::Vector{Int}) where {ST}
    [zeros(ST, W_sizes[d]) for d in 1:D]
end

function GeometricIntegratorsBase.integrate!(
        sol::GeometricSolution, int::GeometricIntegrator{<:VISE}, n₁::Int, n₂::Int)
