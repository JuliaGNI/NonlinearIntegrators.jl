...
begin
    ...
    fig2
    save("HO_hamiltonian_error_TR.pdf", fig2)
end
...
begin
        ...
        for (Si,S) in enumerate(S_list)
            for (ki,k_relu) in enumerate(k_list)
                for (Ri,R) in enumerate(R_list)
                    ...
                    for (fi,f_abs) in enumerate(f_abs_list)
                        for (xi,x_abs) in enumerate(x_abs_list)
                            data_file="default_linesearch/NVI_DP_h$(int_step)S$(S)R$(R)reluk=$(k_relu)fabs$(f_abs)xabs$(x_abs).jld2"
                            isfile(data_file) ? nothing : continue
                            result_data = load(data_file)
    ...
    save("DP_hamiltonian_error.pdf", fig3)
end
...
begin
    ...
    fig4
    save("DP_hamiltonian_error_TR.pdf", fig4)
end
...
begin
    ...
    for (hi,int_step) in enumerate(h_list)
        for (Si,S) in enumerate(S_list)
            for (Ri,R) in enumerate(R_list)
                ...
                for (fi,f_abs) in enumerate(f_abs_list)
                    for (xi,x_abs) in enumerate(x_abs_list)
                        data_file="default_linesearch/NVI_HO_h$(int_step)S$(S)R$(R)fabs$(f_abs)xabs$(x_abs)tanh.jld2"
                        isfile(data_file) ? nothing : continue
                        result_data = load(data_file)
    ...
    fig5
    save("HO_hamiltonian_error_tanh.pdf", fig5)
end
    ...
    for (hi,int_step) in enumerate(h_list)
        for (Si,S) in enumerate(S_list)
            for (Ri,R) in enumerate(R_list)
                ...
                for (fi,f_abs) in enumerate(f_abs_list)
                    for (xi,x_abs) in enumerate(x_abs_list)
                        data_file="default_linesearch/NVI_DP_h$(int_step)S$(S)R$(R)fabs$(f_abs)xabs$(x_abs)tanh.jld2"
                        isfile(data_file) ? nothing : continue
                        result_data = load(data_file)
