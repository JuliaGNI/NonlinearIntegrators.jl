...
precision_colour(T::AbstractString) =
...
read_oga_csv(paths::AbstractVector) = reduce(vcat, (read_oga_csv(p) for p in paths); init = Dict{String,String}[])
...
"""
function labelled_heatmap!(ax, values::Matrix{Float64}; fmt = fmt_e, lo = nothing, hi = nothing)
...
function styled_axis!(fig, pos; title = "", xlabel = "", ylabel = "",
                      ...
                      yscale = identity)
    ax = Axis(fig[pos...]; title = title, xlabel = xlabel, ylabel = ylabel, yscale = yscale,
...
function write_fit_study_report(csvpath::AbstractString)
    ...
    for (metric, key, fmt, transform) in (
        ...
        for (ti, T) in enumerate(Ts), (di, dict) in enumerate(dicts)
            ...
            for (fi, fit) in enumerate(fits), (si, sel) in enumerate(sels)
                sub = [r for r in rows if r["T"] == T && r["dictionary"] == dict &&
                                          r["fit"] == fit && r["selection"] == sel]
                M[fi, si] = oga_median(transform(x) for x in (fnum(r, key) for r in sub)
                                       if isfinite(x) && x > 0)
            end
            panels[(ti, di)] = M
        end
    ...
    open(mdpath, "w") do io
        ...
        for T in Ts; print(io, " $T err | $T cond | $T σ_min |"); end
        println(io)
        print(io, "|---|---|---|")
        for _ in Ts; print(io, "---|---|---|"); end
        println(io)
        ...
        for T in Ts, act in levels(rows, "activation")
            groups = oga_groupby([r for r in rows if r["T"] == T && r["activation"] == act],
                                 variant_label)
        ...
        println(io, "| precision | selection | cases | short of full width | fewest neurons | cases with rejections | most rejected | non-finite |")
        println(io, "|---|---|---|---|---|---|---|---|")
...
function write_sweep_report(csvpaths, name::AbstractString)
    ...
    for (ai, act) in enumerate(acts)
        ...
        for (ti, T) in enumerate(Ts), (si, seed) in enumerate(seeds)
            sub = [r for r in rows if r["T"] == T && r["seed"] == seed && r["activation"] == act]
            isempty(sub) && continue
    ...
    Label(fig2[0, 1:ncol], "$name — accuracy vs regularization factor (0 = no regularization)";
          fontsize = 14, color = OGA_INK, font = :bold)
    for (si, seed) in enumerate(seeds)
        ...
        for T in Ts
            ...
            for m in multiples
                sub = [r for r in rows if r["T"] == T && r["seed"] == seed &&
