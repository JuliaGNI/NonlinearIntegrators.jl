...
function GeometricIntegratorsBase.default_options(method::ShallowNetAutodiff)
    (
        x_suctol = x_suc * eps(),
        f_abstol = f_abs * eps(),
    ...
    for S in S_list
        for k_relu in k_list
            try
                ...
                nlmethod = ShallowNetAutodiff(net, QGau, show_status = false,
                ...
                p = plot(layout = @layout([a; b; c]), label = "", size = (700, 700),
                    plot_title = "HarmonicOscillator,h = $(int_step)")
                plot!(p[1], (int_step / 40):(int_step / 40):int_timespan,
                    vcat(hcat(internal_values...)[2:end, :]...),
                    label = "S$(S)R$(R)k$(k_relu)", ylims = (-0.6, 0.6))
                plot!(p[1], (int_step / 40):(int_step / 40):int_timespan,
                    collect(HO_pref.q[:, 1])[2:end],
                    label = "Analytic Solution", xaxis = "time", yaxis = "q₁")
                plot!(p[2], 0:int_step:int_timespan, collect(HO_sol.p[:, 1]),
                    label = "S$(S)R$(R)k$(k_relu)", ylims = (-0.6, 0.6))
                plot!(p[2], 0:(int_step / 40):int_timespan, collect(HO_pref.p[:, 1]),
                    label = "Analytic Solution", xaxis = "time", yaxis = "p₁")
                plot!(p[3], 0:int_step:int_timespan, relative_hams_err,
                    label = "S$(S)R$(R)k$(k_relu)", xaxis = "time",
                    yaxis = "Relative Hamiltonian error")
                savefig(p,
                    "HC_int_072/NVI_HO_h$(int_step)S$(S)R$(R)reluk=$(k_relu)fabs$(f_abs)xsuc$(x_suc).pdf")
                ...
                save(
                    "HC_int_072/NVI_HO_h$(int_step)S$(S)R$(R)reluk=$(k_relu)fabs$(f_abs)xsuc$(x_suc).jld2",
                    record_results)
                ...
                println(
                    "Error on Harmonic Oscillator, NVI_HO_h$(int_step)S$(S)R$(R)reluk=$(k_relu)fabs$(f_abs)xsuc$(x_suc)",
                    e)
                continue
            end
        end
