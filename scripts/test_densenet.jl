...
pmap([0.05, 0.1, 0.2, 0.5, 1.0]) do int_step
    @info "Testing DenseNet with step size h = $int_step"
    HO_lode = GeometricProblems.HarmonicOscillator.lodeproblem(
        timespan = (0, int_timespan), timestep = int_step)
    HO_pref = HarmonicOscillator.exact_solution(HarmonicOscillator.podeproblem(
        timespan = (0, int_timespan), timestep = int_step))
    HO_pref2 = HarmonicOscillator.exact_solution(HarmonicOscillator.podeproblem(
        timespan = (0, int_timespan), timestep = int_step/40))
    ...
    sigmoid(x) = 1 / (1 + exp(-x))
    relu3 = x -> max(0, x)^3
    net = DenseNetBasis{Float64}(tanh, S₁, S)
    ...
    HO_Dense_sol, internal_values = integrate(HO_lode, nlmethod)
    ...
    hams = [GeometricProblems.HarmonicOscillator.hamiltonian(0, q, p, HO_lode.parameters)
            for (q, p) in zip(collect(HO_Dense_sol.q[:]), collect(HO_Dense_sol.p[:]))]
