...
# ── Block 1: HO + ReLU ───────────────────────────────────────────────────────
# DenseNetBasis takes two hidden-layer widths; S₁ = S for a square network.
...
try
    ...
    elseif any(!isfinite, qend)
        ...
        fname = "NVI_Dense_HO_h$(int_step)S$(S)R$(R)reluk=$(k_relu)reg=$(reg_factor)fabs=$(f_abstol)xsuc=$(x_suctol)_$(solver_name)_$(dtype_str)"
        plot_1d!(outdir, fname, ts_HO, collect(HO_sol.q[:, 1]), collect(HO_sol.p[:, 1]), hams_err,
                 "HO ReLU k=$(k_relu) S$(S)R$(R) Dense h=$(int_step) $(dtype_str)")
...
try
    ...
    elseif any(!isfinite, qend)
        ...
        fname = "NVI_Dense_HO_h$(int_step)S$(S)R$(R)tanh_reg=$(reg_factor)fabs=$(f_abstol)xsuc=$(x_suctol)_$(solver_name)_$(dtype_str)"
        plot_1d!(outdir, fname, ts_HO, collect(HO_sol.q[:, 1]), collect(HO_sol.p[:, 1]), hams_err,
                 "HO tanh S$(S)R$(R) Dense h=$(int_step) $(dtype_str)")
        save_1d_jld2(outdir, fname, collect(HO_sol.q[:, 1]), collect(HO_sol.p[:, 1]),
                     HO_internal, HO_qerror, hams_err; prefix="HO")
    end
catch e
...
if run_dp
    ...
    try
        relu = x -> max(zero(T), x)^k_relu
        net = DenseNetBasis{T}(relu, S, S)
        nlmethod = DenseNet(net, QLob, training_epochs=50000, initial_guess_method=TrainingMethod())
        ...
        elseif any(!isfinite, qend)
            ...
            fname = "NVI_Dense_DP_h$(int_step)S$(S)R$(R)reluk=$(k_relu)reg=$(reg_factor)fabs=$(f_abstol)xsuc=$(x_suctol)_$(solver_name)_$(dtype_str)"
            plot_2d!(outdir, fname, ts_DP,
                     collect(DP_sol.q[:, 1]), collect(DP_sol.q[:, 2]),
                     collect(DP_sol.p[:, 1]), collect(DP_sol.p[:, 2]),
                     DP_hams_err, "DP ReLU k=$(k_relu) S$(S)R$(R) Dense h=$(int_step) $(dtype_str)")
    ...
    try
        ...
        elseif any(!isfinite, qend)
            ...
            fname = "NVI_Dense_DP_h$(int_step)S$(S)R$(R)tanh_reg=$(reg_factor)fabs=$(f_abstol)xsuc=$(x_suctol)_$(solver_name)_$(dtype_str)"
            plot_2d!(outdir, fname, ts_DP,
                     collect(DP_sol.q[:, 1]), collect(DP_sol.q[:, 2]),
                     collect(DP_sol.p[:, 1]), collect(DP_sol.p[:, 2]),
                     DP_hams_err, "DP tanh S$(S)R$(R) Dense h=$(int_step) $(dtype_str)")
            save_2d_jld2(outdir, fname,
                         collect(DP_sol.q[:, 1]), collect(DP_sol.q[:, 2]),
                         collect(DP_sol.p[:, 1]), collect(DP_sol.p[:, 2]),
                         DP_internal, DP_qerror, DP_hams_err; prefix="DP")
