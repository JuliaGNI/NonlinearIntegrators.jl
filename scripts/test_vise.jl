...
# function vise_plot_1d(vise_sol, internal_sol, pref,relative_ham_err, h_step, TT, title_name)
...
#     scatter!(p[1],collect(0:h_step:TT),collect(vise_sol.q[:,1]), label="VISE Discrete Solution")

#     plot!(p[2],collect(0:h_step/40:TT),collect(pref.p[:,1]), label="Reference Solution",xaxis="time", yaxis="p₁")
#     scatter!(p[2],collect(0:h_step:TT),collect(vise_sol.p[:,1]), label="VISE Discrete Solution")
...
# function vise_plot_2d(vise_sol, internal_sol, pref,relative_ham_err, h_step, TT, title_name)
...
#     scatter!(p[1],collect(0:h_step:TT),collect(vise_sol.q[:,1]), label="SINDy Discrete Solution")
...
#     scatter!(p[2],collect(0:h_step:TT),collect(vise_sol.q[:,2]), label="SINDy Discrete Solution")

#     plot!(p[3], 0:h_step:TT, collect(vise_sol.p[:, 1]), label="SINDy Solution", xaxis="time", yaxis="p₁")
#     plot!(p[3], 0:h_step/40:TT, collect(pref.p[:, 1]), label="Reference Solution")

#     plot!(p[4], 0:h_step:TT, collect(vise_sol.p[:, 2]), label="SINDy Solution", xaxis="time", yaxis="p₂")
...
# GeometricIntegrators.Integrators.default_linesearch(method::VISE) =SimpleSolvers.Backtracking()
# # GeometricIntegrators.Integrators.default_linesearch(method::VISE) =SimpleSolvers.Quadratic2()

# GeometricIntegrators.Integrators.default_options(method::VISE) = (
#     # f_abstol = 8eps(),
#     # f_suctol = 2eps(),
...
#         vise_basis = VISEBasis{Float64}([q_expr], [W], ttt,1)
#         TT = 200.0
#         # h_step = 5.0
#         # h_step = ARGS[2]
...
#         vise_method = VISE(vise_basis, QGau4,[[-0.5000433352162222,0.705350078478666,-1.5678140333370576]]) # Pass the init W into the integrator instead of basis
#         # vise_method = VISE(vise_basis, QGau4,[[-0.500,sqrt(0.5),-pi/2]])

#         println("Start to run Harmonic Oscillator Problem with VISE! R = $(R), h = $(h_step)")
#         t1 = time()
#         vise_sol,internal_sol,x_list = integrate(HO_lode, vise_method)
#         t2 = time()
#         hams = [GeometricProblems.HarmonicOscillator.hamiltonian(0, q, p, HO_lode.parameters) for (q, p) in zip(collect(vise_sol.q[:]), collect(vise_sol.p[:]))]
#         relative_hams_err = abs.((hams .- initial_hamiltonian) / initial_hamiltonian)
#         println("\n NISE:")
#         @show relative_maximum_error(vise_sol.q,HO_truth.q)
...
#         # vise_plot_1d(vise_sol, internal_sol, HO_plot, relative_hams_err, h_step, TT, "HarmonicOscillator,h$(h_step)_T$(TT)_R$(R)")
#         # println("Finish integrating Harmonic Oscillator Problem with VISE!, Figure Saved!")
...
#         record_results[("HO_vise_sol_q")] = collect(vise_sol.q[:,1])
#         record_results[("HO_vise_sol_p")] = collect(vise_sol.p[:,1])
#         record_results[("HO_internal_sol")] = internal_sol
#         record_results[("HO_x_list")] = x_list
#         record_results[("HO_qerror")] = relative_maximum_error(vise_sol.q,HO_truth.q)
...
#         vise_basis = VISEBasis{Float64}([q_expr], [W], ttt,1)
#         # R = 4
#         println("Start to run Pendulum Problem with VISE! h = $(h_step), R= $(R)")

#         QGau = QuadratureRules.GaussLegendreQuadrature(R)
#         vise_method = VISE(vise_basis, QGau,[[1.1931,0.45644,-1.1466]])

#         pendulum_vise_sol,pendulum_internal_sol,pendulum_x_list = integrate(pendulum_lode, vise_method)
#         @show relative_maximum_error(pendulum_vise_sol.q,pd_ref_sol.q)

#         pendulum_hams = [GeometricProblems.Pendulum.hamiltonian(0.0, q, p, pendulum_lode.parameters) for (q, p) in zip(collect(pendulum_vise_sol.q[:]), collect(pendulum_vise_sol.p[:]))]
...
#         # vise_plot_1d(vise_sol, internal_sol, sol_plot, relative_hams_err, h_step, TT, "Pendulum,h$(h_step)_T$(TT)_R$(R)")
#         # println("Finish integrating Pendulum Problem with VISE!, Figure Saved!")
...
#         record_results[("Pendulum_vise_sol_q")] = collect(pendulum_vise_sol.q[:,1])
#         record_results[("Pendulum_vise_sol_p")] = collect(pendulum_vise_sol.p[:,1])
#         record_results[("Pendulum_internal_sol")] = pendulum_internal_sol
#         record_results[("Pendulum_x_list")] = pendulum_x_list
#         record_results[("Pendulum_qerror")] = relative_maximum_error(pendulum_vise_sol.q,pd_ref_sol.q)
...
#     ### Perturbed Pendulum
#     begin
#         println("Start to run Perturbed Pendulum Problem with VISE!")
...
#         q_expr = W[1] *cos(W[2]* ttt + W[3])
#         vise_basis = VISEBasis{Float64}([q_expr], [W], ttt,1)
#         vise_method = VISE(vise_basis, QGau,[[-0.51941,-0.47405,2.8713]])
#         println("Start to run Perturbed Pendulum Problem with VISE! R = $(R), h = $(h_step)")
#         pp_vise_sol,pp_internal_sol,pp_x_list = integrate(lode, vise_method)
#         @show relative_maximum_error(vise_sol.q,pp_ref_sol.q)

#         pp_hams = [GeometricProblems.PerturbedPendulum.hamiltonian(0.0, q, p, lode.parameters) for (q, p) in zip(collect(pp_vise_sol.q[:]), collect(pp_vise_sol.p[:]))]
...
#         # vise_plot_1d(vise_sol, internal_sol, sol_plot, relative_hams_err, h_step, TT, "Perturbed_Pendulum,h$(h_step)_T$(TT)_R$(R)")
#         # println("Finish integrating Perturbed Pendulum Problem with VISE!, Figure Saved!")
...
#         record_results[("PerturbedPendulum_vise_sol_q")] = collect(pp_vise_sol.q[:,1])
#         record_results[("PerturbedPendulum_vise_sol_p")] = collect(pp_vise_sol.p[:,1])

#         record_results[("PerturbedPendulum_internal_sol")] = pp_internal_sol
#         record_results[("PerturbedPendulum_x_list")] = pp_x_list
#         record_results[("PerturbedPendulum_qerror")] = relative_maximum_error(pp_vise_sol.q,pp_ref_sol.q)
...
#     #### Henon Heiles
#     begin
#         println("Start to run Henon Heiles Problem with VISE!")
...
#         vise_basis = VISEBasis{Float64}([q₁_expr,q₂_expr], [W1,W2], ttt,2)

#         vise_method = VISE(vise_basis, QGau,[[0.14831,1.0,-0.64812,- 0.018712],[0.14298,- 0.97215,0.7615,-0.0013983]]) # Pass the init W into the integrator instead of basis
#         HH_vise_sol,HH_internal_sol,HH_x_list = integrate(HHlode, vise_method)
#         @show relative_maximum_error(HH_vise_sol.q,HH_ref_sol.q)

#         HH_hams = [GeometricProblems.HenonHeilesPotential.hamiltonian(0.0, q, p, HHlode.parameters) for (q, p) in zip(collect(HH_vise_sol.q[:]), collect(HH_vise_sol.p[:]))]
...
#         # vise_plot_2d(vise_sol, internal_sol, sol_plot,relative_hams_err, h_step, TT, "HenonHeiles_Potential,h$(h_step)_T$(TT)_R$(R)")
#         # println("Finish integrating HenonHeiles Potential Problem with VISE!, Figure Saved!")
...
#         # record_results[("HenonHeiles_vise_sol")] = HH_vise_sol
#         record_results[("HenonHeiles_vise_sol_q1")] = collect(HH_vise_sol.q[:,1])
#         record_results[("HenonHeiles_vise_sol_q2")] = collect(HH_vise_sol.q[:,2])
#         record_results[("HenonHeiles_vise_sol_p1")] = collect(HH_vise_sol.p[:,1])
#         record_results[("HenonHeiles_vise_sol_p2")] = collect(HH_vise_sol.p[:,2])

#         record_results[("HenonHeiles_internal_sol")] = HH_internal_sol
#         record_results[("HenonHeiles_x_list")] = HH_x_list
#         record_results[("HenonHeiles_qerror")] = relative_maximum_error(HH_vise_sol.q,HH_ref_sol.q)
...
#         scatter!(q2_axis, t_coarse, collect(vise_sol.p[:,1]), label="VISE Discrete Solution ",color = :blue)
...
#         scatter!(q1_axis,t_coarse, collect(pendulum_vise_sol.q[:,1]), label="VISE Discrete Solution",color = :blue)
...
#         scatter!(q2_axis, t_coarse, collect(pendulum_vise_sol.p[:,1]), label="VISE Discrete Solution ",color = :blue)
...
#         scatter!(q1_axis,t_coarse, collect(pp_vise_sol.q[:,1]), label="VISE Discrete Solution",color = :blue)
#         vlines!(q1_axis,[100.0],linestyle=:dashdot,color = :purple,label = "Training Region")
...
#         scatter!(q2_axis, t_coarse, collect(pp_vise_sol.p[:,1]), label="VISE Discrete Solution ",color = :blue)
...
#         scatter!(q1_axis,t_coarse, collect(HH_vise_sol.q[:,1]), label="VISE Discrete Solution",color = :blue)
...
#         scatter!(q2_axis,t_coarse, collect(HH_vise_sol.q[:,2]), label="VISE Discrete Solution",color = :blue)
...
TT = 1.0
h = 1.0
# Gaussian initial condition (mean μ, standard deviation σ)
...
@variables ttt
@variables W1[1:4] W2[1:5] W3[1:5] W4[1:5] W5[1:7] W6[1:5]
