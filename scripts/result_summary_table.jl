...
begin
    HO_err_tensor = zeros(
        length(h_list), length(S_list), length(k_list), length(R_list), length(λ_list))
    q_err_tensor = zeros(
        length(h_list), length(S_list), length(k_list), length(R_list), length(λ_list))
                ...
                for (Ri, R) in enumerate(R_list)
                    ...
                    for (λi, λ) in enumerate(λ_list)
                        ...
                        try
                            ...
                            HO_err_tensor[
                                hi, Si, ki, Ri, λi] = result_data["HO_max_hams_err"]
    ...
    for (Si, S) in enumerate(S_list)
        for (ki, k_relu) in enumerate(k_list)
            ...
            scatterlines!(ax, h_list, err_mean, label = "S$(S)k$(k_relu)",
                color = line_colors[line_idx],
                markersize = 6, linewidth = 2)
            errorbars!(ax, h_list, err_mean, errlow, errhigh, linewidth = 2,
                color = line_colors[line_idx], whiskerwidth = 10)

            scatterlines!(ax2, h_list, err_min, label = "S$(S)k$(k_relu)",
                color = line_colors[line_idx],
                markersize = 6, linewidth = 2)
