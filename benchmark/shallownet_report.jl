# Reporting for the `ShallowNet` benchmark.
#
...
function read_results(paths::AbstractVector{<:AbstractString})
    ...
    for path in paths
        ...
        for ln in lines[2:end]
            ...
            push!(rows,
                (problem = String(f[1]), T = String(f[2]), dt = _parsef(f[3]),
...
function group_stats(rows)
    ...
    (n = n, ok = ok, frac = n == 0 ? 0.0 : ok / n, measured = length(mrows),
        ...
        med_secs = _median_finite([r.total_secs for r in mrows]))
end
...
function plot_success_bars(rows, keyfn, xlabel, title, path)
    ...
    text!(ax, 1:length(labels), fracs; text = fmt_pct.(fracs),
...
function plot_success_heatmap(rows, path; keyfn = strategy_label, keylabel = "solver")
    isempty(rows) && return false
    groups = sort(unique(keyfn.(rows)))
    precisions = sort(unique(r.T for r in rows))
    M = fill(NaN, length(groups), length(precisions))
    for (i, s) in enumerate(groups), (j, p) in enumerate(precisions)

        sub = [r for r in rows if keyfn(r) == s && r.T == p]
        isempty(sub) || (M[i, j] = group_stats(sub).frac)
    end
    fig = Figure(size = (150 * length(precisions) + 300, 90 * length(groups) + 150))
    ax = Axis(fig[1, 1]; title = "Convergence: success rate by $(keylabel) × precision",
        xticks = (1:length(precisions), precisions),
        yticks = (1:length(groups), groups))
    hm = heatmap!(ax, 1:length(precisions), 1:length(groups), permutedims(M);
        colormap = cgrad([CONV_RED, CONV_GREEN]), colorrange = (0, 1))
    for (i, s) in enumerate(groups), (j, p) in enumerate(precisions)

        isnan(M[i, j]) ||
            text!(ax, j, i; text = fmt_pct(M[i, j]), align = (:center, :center),
                color = :white)
    end
...
function plot_metric_vs_dt(rows, field, ylabel, title, path;
    ...
    data = [(r.dt, getv(r), string(getproperty(r, colorby)))
            for r in rows
            if has_metrics(r) && isfinite(getv(r)) && isfinite(r.dt) &&
                   (!ylog || getv(r) > 0)]
...
function _stats_table(io, rows, keyfn, colname)
    ...
    _table(io,
        [colname, "n", "ok", "success", "measured", "med ref_err", "med ham_drift",
            "med iter", "med total_s"],
        cells)
end
...
function _best_configs(io, rows)
    ...
    for (prob, v) in groupby(rows, r -> r.problem)
        ...
        push!(rowsout,
            [string(prob), fmt_sci(b.ref_err), b.T, @sprintf("%.3g", b.dt),
