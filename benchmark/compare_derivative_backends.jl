...
function mkcodegen(label, suffix, cse, inplace)
...
function basis_key(::Type{T}, actlabel, S, cg::NamedTuple, symbolic::Bool) where {T}
    (string(T), actlabel, S, symbolic, cg.cse, cg.inplace)
end

function get_basis(
        ::Type{T}, act, actlabel, S::Int, cg::NamedTuple, symbolic::Bool) where {T}
...
function basis_secs(::Type{T}, actlabel, S, cg, symbolic) where {T}
    get(BASIS_SECS, basis_key(T, actlabel, S, cg, symbolic), NaN)
end

# ---- CSV --------------------------------------------------------------------

const BACKEND_CSV_HEADER = "method,backend,codegen,problem,T,dt,steps,R,S,activation,solver,linesearch," *
                           "initial_guess,seed,lambda,status,ref_err,ham_drift,iterations,warm_secs,cold_secs,basis_secs"

const KERNEL_CSV_HEADER = "T,S,activation,kernel,backend,codegen,calls,min_secs,median_secs,bytes"
...
function read_backend_results(path::AbstractString)
    ...
    for ln in lines[2:end]
        ...
        push!(rows,
            (method = String(f[1]), backend = String(f[2]), codegen = String(f[3]),
...
function read_agreement_results(path::AbstractString)
    ...
    for ln in lines[2:end]
        ...
        length(f) == 5 || continue
        push!(rows,
            (T = String(f[1]), S = round(Int, _parsef(f[2])),
        ...
        for spec in cfg.problems
            ...
            for T in cfg.types, (actlabel, act) in cfg.activations
                ...
                for v in VARIANTS
                    ...
                    method = v.ctor(
                        basis, QuadratureRules.GaussLegendreQuadrature(T, spec.R);
                    ...
                    for dt in cfg.dts
                        ...
                        cold = run_case(
                            prob, method, T, IG, STRAT, λ, nothing, refq, spec.ham, params)
                        warm = run_case(
                            prob, method, T, IG, STRAT, λ, nothing, refq, spec.ham, params)
                        ...
                        @printf("%-38s %-20s %-8s %-6.3g | %-10s %-10s %-10s %-5s %-9s %-9s %-9s\n",
                            ...
                            isnan(warm.total_secs) ? "—" :
                            @sprintf("%.3f", warm.total_secs),
                            isnan(cold.total_secs) ? "—" :
                            @sprintf("%.3f", cold.total_secs),
                            isnan(bsecs) ? "—" : @sprintf("%.3f", bsecs))

                        row = join(
                            (v.label, v.backend, v.codegen.label, spec.name, string(T),
...
function kernel_params(::Type{T}, S::Int) where {T}
    ...
    ps = (
        L1 = (W = rand(rng, T, S, 1), b = rand(rng, T, S)), L2 = (W = rand(rng, T, 1, S),))
    ...
    open(agrpath, "w") do agr
        ...
        open(csvpath, "w") do io
            ...
            for T in cfg.types, S in cfg.kernel_Ss
                ...
                q̄ = T(0.5)
                q = T(0.7)           # the endpoint values the autodiff ansatz needs
                input = [t]
                ...
                for (kernel, backend, codegen, f) in entries
                    ...
                    @printf("%-8s %-4d %-8s %-10s %-12s | %-12.3f %-12.3f %-10d\n",
                        string(T), S, kernel, backend, codegen, 1e6 * r.min,
                        1e6 * r.median,
                        round(Int, r.bytes))
                    println(io,
                        join(
                            (string(T), csvnum(S), actlabel, kernel, backend, codegen,
...
function _cell(rows, problem, activation, dt, method)
    hit = findfirst(
        r -> r.problem == problem && r.activation == activation &&
             r.dt == dt && r.method == method,
        rows)
    hit === nothing && return "—"
    r = rows[hit]
...
function _codegen_endtoend_spread(io, rows)
    ...
    for v in VARIANTS
        ...
        for r in rows
            ...
            hit = findfirst(
                s -> s.method == base.label && s.problem == r.problem &&
        ...
        push!(cells,
            [base.base, string(length(pairs)),
                isnan(maxrel) ? "—" : @sprintf("%.1e", maxrel),
                string(iterdiff), mismatched == 0 ? "yes" : "no ($(mismatched))"])
    end
    _table(io,
        ["integrator", "paired cases", "max rel. Δ ref_err",
            "cases with a different iteration count",
            "statuses agree"],
        cells)
end
...
function write_backend_report(rows, krows, arows; mode, outdir)
    ...
    open(md, "w") do io
        ...
        println(io,
            "- Total cases: **$(ntot)**  •  converged (`ok`): **$(nok)** " *
        ...
        _table(io,
            ["integrator", "backend", "codegen", "med basis build [s]", "med cold [s]",
                "med warm [s]", "med iter"],
            cost)
        ...
        cells = [[p, T, a, @sprintf("%.3g", dt),
                     (_cell([r for r in rows if r.T == T], p, a, dt, m)
                     for m in headtohead)...]
...
function _kernel_tables(io, krows)
    pick(T, S, kernel, backend, codegen) = findfirst(
        r -> r.T == T && r.S == S && r.kernel == kernel &&
             r.backend == backend && r.codegen == codegen,
        krows)
    ...
    for T in sort(unique(r.T for r in krows)),
        ...
        push!(bytes,
            [T, string(S), kernel, string(round(Int, d.bytes)),
                string(round(Int, p.bytes)), string(round(Int, a.bytes))])
    end
    println(io, "### Time per call (median)\n")
    _table(io,
        ["T", "S", "kernel", "cse+inplace [µs]", "plain [µs]", "autodiff [µs]",
            "plain ÷ cse+inplace", "autodiff ÷ cse+inplace"],
        times)
