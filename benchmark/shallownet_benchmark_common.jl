# Shared engine for the `ShallowNet` benchmark suite.
#
...
include(joinpath(@__DIR__, "shallownet_report.jl"))     # read_results / plots / markdown
...
pick_mode() = get(ENV, "SHALLOWNET_BENCH_PRESET", isempty(ARGS) ? "quick" : ARGS[1])
